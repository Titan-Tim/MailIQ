/**
 * Generate the Mail-IQ QR document-separator sheet.
 *   node scripts/gen-separator-sheet.js
 * Staff print a stack and place one between each document in a batch scan.
 */
const fs = require('fs')
...
async function main() {
  ...
  const targets = [
    'C:\\Mail-IQ Demo Documents\\Separator Sheet',
    path.join(__dirname, '..', '..', 'agent'),
  ]
  for (const dir of targets) {
    fs.mkdirSync(dir, { recursive: true })
    const out = path.join(dir, 'mailiq-separator-sheet.pdf')
